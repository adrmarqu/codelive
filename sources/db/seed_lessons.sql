-- ============================================================
--  CodeLive — Seed de lecciones de prueba (un nivel por idioma)
--  Ejecutar DESPUÉS de init.sql y con los módulos ya insertados.
--  Usa subqueries para ser robusto ante cambios de IDs.
-- ============================================================
...
DECLARE
    ...
    -- Lesson IDs (returned from INSERT)
    v_html_les  INT;
    v_css_les   INT;
    v_js_les    INT;
    v_php_les   INT;
...
BEGIN
    ...
    -- ── 1. Resolve module IDs ─────────────────────────────────
    SELECT m.id INTO v_html_mod
        FROM modules m JOIN courses c ON m.id_course = c.id
        WHERE c.name = 'HTML' AND m.name = 'Introducción';

    SELECT m.id INTO v_css_mod
        FROM modules m JOIN courses c ON m.id_course = c.id
        WHERE c.name = 'CSS' AND m.name = 'Introducción';

    SELECT m.id INTO v_js_mod
        FROM modules m JOIN courses c ON m.id_course = c.id
        WHERE c.name = 'JavaScript' AND m.name = 'Introducción';

    SELECT m.id INTO v_php_mod
        FROM modules m JOIN courses c ON m.id_course = c.id
        WHERE c.name = 'PHP' AND m.name = 'Introducción';

    SELECT m.id INTO v_node_mod
        FROM modules m JOIN courses c ON m.id_course = c.id
        WHERE c.name = 'NodeJS' AND m.name = 'Introducción';

    SELECT m.id INTO v_sql_mod
        FROM modules m JOIN courses c ON m.id_course = c.id
        WHERE c.name = 'MySQL' AND m.name = 'Introducción';

    -- ── 2. Create lessons (level 1 in each intro module) ─────
    INSERT INTO lessons (level, id_module) VALUES (1, v_html_mod) RETURNING id INTO v_html_les;
    INSERT INTO lessons (level, id_module) VALUES (1, v_css_mod)  RETURNING id INTO v_css_les;
    INSERT INTO lessons (level, id_module) VALUES (1, v_js_mod)   RETURNING id INTO v_js_les;
    INSERT INTO lessons (level, id_module) VALUES (1, v_php_mod)  RETURNING id INTO v_php_les;
    INSERT INTO lessons (level, id_module) VALUES (1, v_node_mod) RETURNING id INTO v_node_les;
    INSERT INTO lessons (level, id_module) VALUES (1, v_sql_mod)  RETURNING id INTO v_sql_les;

    -- ── 3. Insert lesson content ──────────────────────────────
...
Pulsa Ejecutar para ver la salida en el terminal. Prueba a añadir más console.log() o a modificar el array.',
    ...
    INSERT INTO lesson_content (id_lesson, title, content, code, code_lang) VALUES (
        ...
        'SQL (Structured Query Language) es el lenguaje estándar para gestionar bases de datos relacionales. Se usa en prácticamente todos los sistemas que almacenan datos: desde apps móviles hasta grandes empresas.
...
En este entorno de práctica tienes acceso a los datos reales de CodeLive (usuarios, cursos, módulos). Todos los cambios se descartan automáticamente al terminar. Prueba a añadir un WHERE, un ORDER BY o un LIMIT.',
        E'-- Consulta los usuarios registrados en CodeLive\nSELECT\n    username,\n    email,\n    rol,\n    created_at\nFROM users\nORDER BY created_at DESC;',
        'sql'
    );

    RAISE NOTICE 'Lecciones de prueba insertadas correctamente.';
END $$;
